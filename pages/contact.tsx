...
const Contact: FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToast({ type: 'success', msg: '메시지가 전송되었습니다. 빠르게 확인하겠습니다!' });
      setFormData({ name: '', email: '', message: '', website: '' });
    } catch (err: any) {
      setToast({ type: 'error', msg: err.message || '전송 중 오류가 발생했습니다.' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      ...
      <section className="py-16 bg-gray-100 text-gray-900 flex flex-col items-center justify-center min-h-screen">
        <div className="max-w-4xl w-full px-6 text-center">
          ...
          <p className="text-sm text-gray-600 mb-8">아래 폼을 작성해 주시면 메일로 전달됩니다.</p>

          <form className="bg-white p-8 rounded-lg shadow-md w-full max-w-lg mx-auto text-left" onSubmit={handleSubmit}>
            {/* 허니팟 필드 (봇 차단용) */}
            <input
              type="text"
              name="website"
            ...
            {toast && (
              <p className={`mt-4 text-sm ${toast.type === 'success' ? 'text-green-600' : 'text-red-600'}`}>
                {toast.msg}
              </p>
            )}
          </form>
